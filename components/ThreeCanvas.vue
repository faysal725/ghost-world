...
onMounted(() => {
  ...
  const bricsColorTexture = textureLoader.load(
    "/textures/bricks/color.jpg"
  );
  const bricsNormalTexture = textureLoader.load(
    "/textures/bricks/normal.jpg"
  );
  const bricsAmbientOcclusionTexture = textureLoader.load(
    "/textures/bricks/ambientOcclusion.jpg"
  );
  const bricsRoughnessTexture = textureLoader.load(
  ...
  const walls = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
      map: bricsColorTexture,
      transparent: true,
      aoMap: bricsAmbientOcclusionTexture,
      normalMap: bricsNormalTexture,
      roughness: bricsRoughnessTexture,
    })
  );
  ...
  // gui.add(door.rotation, "y").min(0).max(2).step(0.001).name('rotation');
  house.add(door);
  ...
  bush1.position.set(0.8, 0.3, 2.5);
  ...
  gui.add(doorLight, "intensity").min(0).max(10).step(0.001).name("dorrlight");
  house.add(doorLight);
